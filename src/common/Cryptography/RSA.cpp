...
struct HMAC_SHA256_MD
{
    ...
    HMAC_SHA256_MD()
    {
        ...
        EVP_MD_meth_set_result_size(_md, Acore::Crypto::Constants::SHA256_DIGEST_LENGTH_BYTES);
    ...
    static int Copy(EVP_MD_CTX* to, EVP_MD_CTX const* from)
    {
        ...
        if (ctxDataFrom->hmac)
            ctxDataTo->hmac = new Acore::Crypto::HMAC_SHA256(*ctxDataFrom->hmac);
...
private:
    ...
    static int DigestInit(void* dctx, OSSL_PARAM const* params)
    {
        ...
        if (OSSL_PARAM const* keyParam = OSSL_PARAM_locate_const(params, "hmac-key"))
        {
            ...
            {
                ctxData->hmac = new Acore::Crypto::HMAC_SHA256(key, keyLength);
                return 1;
            }
        }
    ...
    static int DigestGetParams(OSSL_PARAM params[])
    {
        ...
        if (p != nullptr && !OSSL_PARAM_set_size_t(p, Acore::Crypto::Constants::SHA256_DIGEST_LENGTH_BYTES))
            return 0;
