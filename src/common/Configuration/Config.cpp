...
namespace
{
    ...
    bool IsAppConfig(std::string_view fileName)
    {
        ...
        std::size_t foundBnet = fileName.find("bnetserver.conf");

        return foundAuth != std::string_view::npos || foundWorld != std::string_view::npos || foundImport != std::string_view::npos || foundBnet != std::string_view::npos;
    }
