...
class LoginRESTService : public Acore::Net::Http::HttpService<LoginHttpSession>
{
public:
    using RequestHandlerResult = Acore::Net::Http::RequestHandlerResult;
    using HttpRequest = Acore::Net::Http::Request;
    using HttpResponse = Acore::Net::Http::Response;
    using HttpRequestContext = Acore::Net::Http::RequestContext;
    using HttpSessionState = Acore::Net::Http::SessionState;

    LoginRESTService() : HttpService("login"), _port(0), _loginTicketDuration(0) { }

    static LoginRESTService& Instance();

    bool StartNetwork(Acore::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port, int32 threadCount = 1) override;

    std::string const& GetHostnameForClient(boost::asio::ip::address const& address) const;
    uint16 GetPort() const { return _port; }

    std::shared_ptr<Acore::Net::Http::SessionState> CreateNewSessionState(boost::asio::ip::address const& address) override;
...
private:
    ...
    static std::unique_ptr<Acore::Crypto::Bnet::BnetSRP6Base> CreateSrpImplementation(SrpVersion version, SrpHashFunction hashFunction,
