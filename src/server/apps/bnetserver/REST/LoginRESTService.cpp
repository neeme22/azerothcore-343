...
bool LoginRESTService::StartNetwork(Acore::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port, int32 threadCount)
{
    ...
    _hostnames[0] = sConfigMgr->GetOption<std::string>("LoginREST.ExternalAddress", "127.0.0.1");
    ...
    if (!externalAddress)
    {
        LOG_ERROR("server.http.login", "Could not resolve LoginREST.ExternalAddress {}", _hostnames[0]);
        return false;
    }

    _addresses[0] = externalAddress->address();

    _hostnames[1] = sConfigMgr->GetOption<std::string>("LoginREST.LocalAddress", "127.0.0.1");
    Optional<boost::asio::ip::tcp::endpoint> localAddress = resolver.Resolve(boost::asio::ip::tcp::v4(), _hostnames[1], std::to_string(_port));
    if (!localAddress)
    {
        LOG_ERROR("server.http.login", "Could not resolve LoginREST.LocalAddress {}", _hostnames[1]);
        return false;
    }
    ...
    _loginTicketDuration = sConfigMgr->GetOption<int32>("LoginREST.TicketDuration", 3600);
...
{
    if (auto addressIndex = Acore::Net::SelectAddressForClient(address, _addresses))
...
LoginRESTService::RequestHandlerResult LoginRESTService::HandleGetForm(std::shared_ptr<LoginHttpSession> session, HttpRequestContext& context) const
{
    ...
    form.set_srp_url(Acore::StringFormat("https://{}:{}/bnetserver/login/srp/", GetHostnameForClient(session->GetRemoteIpAddress()), _port));
...
LoginRESTService::RequestHandlerResult LoginRESTService::HandlePostLogin(std::shared_ptr<LoginHttpSession> session, HttpRequestContext& context) const
{
        ...
        .WithChainingPreparedCallback([this, session, context = std::move(context), loginForm = std::move(loginForm), getInputValue](QueryCallback& callback, PreparedQueryResult result) mutable
    {
        ...
        // === AzerothCore: las cuentas son las de `account` (usuario + SRP6 SHA-1 de AC). Sin SRP web.
        uint32 gameAccountId = accountId;
        {
            std::string password(getInputValue(loginForm.get(), "password"));
            Utf8ToUpperOnlyLatin(password);
            Acore::Crypto::SRP6::Salt s = fields[2].Get<Binary, Acore::Crypto::SRP6::SALT_LENGTH>();
            Acore::Crypto::SRP6::Verifier v = fields[3].Get<Binary, Acore::Crypto::SRP6::VERIFIER_LENGTH>();
            passwordCorrect = Acore::Crypto::SRP6::CheckLogin(login, password, s, v);

            // fila de battlenet_accounts enlazada 1:1 con la cuenta; se crea la primera vez
            accountId = fields[8].IsNull() ? 0 : fields[8].Get<uint32>();
            if (!accountId)
            {
                LoginDatabasePreparedStatement* ins = LoginDatabase.GetPreparedStatement(LOGIN_INS_BNET_ACCOUNT_FOR_ACCOUNT);
                ins->SetData(0, login);
                ins->SetData(1, std::vector<uint8>(s.begin(), s.end()));
                ins->SetData(2, std::vector<uint8>(v.begin(), v.end()));
                LoginDatabase.DirectExecute(ins);
                LoginDatabasePreparedStatement* sel = LoginDatabase.GetPreparedStatement(LOGIN_SEL_BNET_ACCOUNT_ID_BY_EMAIL);
                sel->SetData(0, login);
                if (PreparedQueryResult idResult = LoginDatabase.Query(sel))
                    accountId = (*idResult)[0].Get<uint32>();
                if (accountId)
                {
                    LoginDatabasePreparedStatement* upd = LoginDatabase.GetPreparedStatement(LOGIN_UPD_ACCOUNT_LINK_BNET);
                    upd->SetData(0, accountId);
                    upd->SetData(1, gameAccountId);
                    LoginDatabase.DirectExecute(upd);
                    LOG_INFO("server.http.login", "Cuenta {} ({}) enlazada a battlenet_accounts {}", login, gameAccountId, accountId);
                }
            }
        }
        ...
        stmt->SetData(1, time(nullptr) + _loginTicketDuration);
...
LoginRESTService::RequestHandlerResult LoginRESTService::HandlePostLoginSrpChallenge(std::shared_ptr<LoginHttpSession> session, HttpRequestContext& context)
{
        ...
        .WithPreparedCallback([session, context = std::move(context), login = std::move(login)](PreparedQueryResult result) mutable
    {
        ...
        Acore::Crypto::Bnet::Salt s = fields[1].Get<Binary, Acore::Crypto::Bnet::SALT_LENGTH>();
        Acore::Crypto::Bnet::Verifier v = fields[2].Get<Binary>();
...
std::unique_ptr<Acore::Crypto::Bnet::BnetSRP6Base> LoginRESTService::CreateSrpImplementation(SrpVersion version, SrpHashFunction hashFunction,
    std::string const& username, Acore::Crypto::Bnet::Salt const& salt, Acore::Crypto::Bnet::Verifier const& verifier)
{
    if (version == SrpVersion::v2)
    {
        if (hashFunction == SrpHashFunction::Sha256)
            return std::make_unique<Acore::Crypto::Bnet::BnetSRP6v2<Acore::Crypto::SHA256>>(username, salt, verifier);
        if (hashFunction == SrpHashFunction::Sha512)
            return std::make_unique<Acore::Crypto::Bnet::BnetSRP6v2<Acore::Crypto::SHA512>>(username, salt, verifier);
    }

    if (version == SrpVersion::v1)
    {
        if (hashFunction == SrpHashFunction::Sha256)
            return std::make_unique<Acore::Crypto::Bnet::BnetSRP6v1<Acore::Crypto::SHA256>>(username, salt, verifier);
        if (hashFunction == SrpHashFunction::Sha512)
            return std::make_unique<Acore::Crypto::Bnet::BnetSRP6v1<Acore::Crypto::SHA512>>(username, salt, verifier);
    }
...
void LoginRESTService::MigrateLegacyPasswordHashes() const
{
    ...
    LOG_INFO(_logger, "Updating password hashes...");
    ...
    if (!result)
    {
        LOG_INFO(_logger, ">> No password hashes to update - this took us {} ms to realize", GetMSTimeDiffToNow(start));
        return;
    }
    ...
    do
    {
        ...
        Acore::Crypto::Bnet::Salt salt = Acore::Crypto::GetRandomBytes<Acore::Crypto::Bnet::SALT_LENGTH>();
        ...
        Acore::Crypto::Bnet::Verifier verifier = Acore::Crypto::Bnet::BnetSRP6v1Base::g.ModExp(x, Acore::Crypto::Bnet::BnetSRP6v1Base::N).ToByteVector();
    ...
    LOG_INFO(_logger, ">> {} password hashes updated in {} ms", c, GetMSTimeDiffToNow(start));
}
}
