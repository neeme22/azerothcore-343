...
void LoginHttpSession::Start()
{
    ...
    LOG_TRACE("server.http.session", "{} Accepted connection", GetClientInfo());
...
void LoginHttpSession::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        if (banned)
        {
            LOG_DEBUG("server.http.session", "{} tries to log in using banned IP!", GetClientInfo());
            CloseSocket();
            return;
        }
    }
...
Acore::Net::Http::RequestHandlerResult LoginHttpSession::RequestHandler(Acore::Net::Http::RequestContext& context)
{
    return sLoginService.HandleRequest(shared_from_this(), context);
}

std::shared_ptr<Acore::Net::Http::SessionState> LoginHttpSession::ObtainSessionState(Acore::Net::Http::RequestContext& context) const
{
    using namespace std::string_literals;

    std::shared_ptr<Acore::Net::Http::SessionState> state;

    auto cookieItr = context.request.find(boost::beast::http::field::cookie);
    if (cookieItr != context.request.end())
    {
        std::vector<std::string_view> cookies = Acore::Tokenize(Acore::Net::Http::ToStdStringView(cookieItr->value()), ';', false);
        std::size_t eq = 0;
    ...
    if (!state)
    {
        ...
        std::string_view host = Acore::Net::Http::ToStdStringView(context.request[boost::beast::http::field::host]);
        if (std::size_t port = host.find(':'); port != std::string_view::npos)
            host.remove_suffix(host.length() - port);

        context.response.insert(boost::beast::http::field::set_cookie, Acore::StringFormat("{}={}; Path=/bnetserver; Domain={}; Secure; HttpOnly; SameSite=None",
