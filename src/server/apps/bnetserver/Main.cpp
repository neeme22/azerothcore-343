...
void ServiceStatusWatcher(std::weak_ptr<Acore::Asio::DeadlineTimer> serviceStatusWatchTimerRef, std::weak_ptr<Acore::Asio::IoContext> ioContextRef, boost::system::error_code const& error);
#endif
...
int main(int argc, char** argv)
{
    ...
    auto configFile = fs::absolute(fs::path(sConfigMgr->GetConfigPath() + std::string("bnetserver.conf")));
    ...
    if (!pidFile.empty())
    {
        ...
        {
            LOG_ERROR("server.bnetserver", "Cannot create PID file {}.\n", pidFile);
            return 1;
        }
    }
...
#if AC_PLATFORM == AC_PLATFORM_WINDOWS
    std::shared_ptr<Acore::Asio::DeadlineTimer> serviceStatusWatchTimer;
    if (m_ServiceStatus != -1)
    {
        serviceStatusWatchTimer = std::make_shared<Acore::Asio::DeadlineTimer>(*ioContext);
...
void ServiceStatusWatcher(std::weak_ptr<Acore::Asio::DeadlineTimer> serviceStatusWatchTimerRef, std::weak_ptr<Acore::Asio::IoContext> ioContextRef, boost::system::error_code const& error)
{
    if (!error)
    {
        if (std::shared_ptr<Acore::Asio::IoContext> ioContext = ioContextRef.lock())
        {
            ...
            else if (std::shared_ptr<Acore::Asio::DeadlineTimer> serviceStatusWatchTimer = serviceStatusWatchTimerRef.lock())
            {
                serviceStatusWatchTimer->expires_after(std::chrono::seconds(1));
