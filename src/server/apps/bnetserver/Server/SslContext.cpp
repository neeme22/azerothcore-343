...
{
    return Acore::make_unique_ptr_with_deleter(UI_UTIL_wrap_read_pem_callback(callback, 0), ::UI_destroy_method);
}
...
auto OpenOpenSSLStore(boost::filesystem::path const& storePath, UI_METHOD const* passwordCallback, void* passwordCallbackData)
{
    ...
    return Acore::make_unique_ptr_with_deleter(OSSL_STORE_open(uri.c_str(), passwordCallback, passwordCallbackData, nullptr, nullptr), ::OSSL_STORE_close);
}
...
#define LOAD_CHECK(fn) do { fn; \
    if (err) \
    { \
        LOG_ERROR("server.ssl", #fn " failed: {}", err.message()); \
        return false; \
    } } while (0)

    std::string certificateChainFile = sConfigMgr->GetOption<std::string>("CertificatesFile", "./bnetserver.cert.pem");

    auto passwordCallback = [](std::size_t /*max_length*/, boost::asio::ssl::context::password_purpose /*purpose*/) -> std::string
    {
        return sConfigMgr->GetOption<std::string>("PrivateKeyPassword", "");
    };
    ...
    if (!store)
    {
        ...
        LOG_ERROR("server.ssl", "OSSL_STORE_open failed: {}", err.message());
        return false;
    }
    ...
    if (!key)
    {
        std::string privateKeyFile = sConfigMgr->GetOption<std::string>("PrivateKeyFile", "./bnetserver.key.pem");
