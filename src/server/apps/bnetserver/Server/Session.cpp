...
void Battlenet::Session::AccountInfo::LoadResult(PreparedQueryResult result)
{
    ...
    LoginTicketExpiry = fields[5].Get<uint32>();
    IsBanned = fields[6].Get<uint64>() != 0;
    IsPermanenetlyBanned = fields[7].Get<uint64>() != 0;

    static constexpr uint32 GameAccountFieldsOffset = 8;

    do
    {
        GameAccounts[result->Fetch()[GameAccountFieldsOffset].Get<uint32>()].LoadResult(result->Fetch() + GameAccountFieldsOffset);
...
void Battlenet::Session::GameAccountInfo::LoadResult(Field const* fields)
{
    ...
    UnbanDate = fields[2].Get<uint32>();
    IsPermanenetlyBanned = fields[3].Get<uint32>() != 0;
    IsBanned = IsPermanenetlyBanned || UnbanDate > time(nullptr);
    SecurityLevel = AccountTypes(fields[4].Get<uint8>());
...
void Battlenet::Session::Start()
{
    ...
    LOG_TRACE("session", "{} Accepted connection", GetClientInfo());
...
void Battlenet::Session::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        if (banned)
        {
            LOG_DEBUG("session", "{} tries to log in using banned IP!", GetClientInfo());
            CloseSocket();
            return;
        }
    }
...
uint32 Battlenet::Session::HandleLogon(authentication::v1::LogonRequest const* logonRequest, std::function<void(ServiceBase*, uint32, ::google::protobuf::Message const*)>& continuation)
{
    ...
    {
        LOG_DEBUG("session", "[Battlenet::LogonRequest] {} attempted to log in with game other than WoW (using {})!", GetClientInfo(), logonRequest->program());
        return ERROR_BAD_PROGRAM;
    }

    if (logonRequest->platform() != "Win" && logonRequest->platform() != "Wn64" && logonRequest->platform() != "Mc64")
    {
        LOG_DEBUG("session", "[Battlenet::LogonRequest] {} attempted to log in from an unsupported platform (using {})!", GetClientInfo(), logonRequest->platform());
        return ERROR_BAD_PLATFORM;
    }

    if (GetLocaleByName(logonRequest->locale()) >= TOTAL_LOCALES)
    {
        LOG_DEBUG("session", "[Battlenet::LogonRequest] {} attempted to log in with unsupported locale (using {})!", GetClientInfo(), logonRequest->locale());
    ...
    _timezoneOffset = [&]
    {
        ...
        return Acore::Timezone::GetOffsetByHash(itr->value.GetUint());
    }();
    ...
    externalChallenge.set_payload(Acore::StringFormat("https://{}:{}/bnetserver/login/", sLoginService.GetHostnameForClient(GetRemoteIpAddress()), sLoginService.GetPort()));
...
uint32 Battlenet::Session::HandleGenerateWebCredentials(authentication::v1::GenerateWebCredentialsRequest const* request, std::function<void(ServiceBase*, uint32, google::protobuf::Message const*)>& continuation)
{
    ...
    if (request->program() != 0x576F57)
    {
        ...
        LOG_DEBUG("session", "[Battlenet::HandleGenerateWebCredentials] {} attempted to generate web cretentials with game other than WoW (using {}{}{}{})!",
    ...
    _queryProcessor.AddCallback(LoginDatabase.AsyncQuery(stmt).WithPreparedCallback([this, asyncContinuation = std::move(continuation)](PreparedQueryResult result)
    {
        ...
        response.set_web_credentials((*result)[0].Get<std::string>().c_str());
        ...
        .WithChainingPreparedCallback([accountInfo](QueryCallback& callback, PreparedQueryResult characterCountsResult)
    {
        if (characterCountsResult)
        {
            do
            {
                ...
                accountInfo->GameAccounts[fields[0].Get<uint32>()]
                    .CharacterCounts[Battlenet::RealmHandle{ fields[3].Get<uint8>(), fields[4].Get<uint8>(), fields[2].Get<uint32>() }.GetAddress()] = fields[1].Get<uint8>();
        ...
        .WithPreparedCallback([this, accountInfo, asyncContinuation](PreparedQueryResult lastPlayerCharactersResult)
    {
        if (lastPlayerCharactersResult)
        {
            do
            {
                ...
                Battlenet::RealmHandle realmId{ fields[1].Get<uint8>(), fields[2].Get<uint8>(), fields[3].Get<uint32>() };
                Battlenet::Session::LastPlayedCharacterInfo& lastPlayedCharacter = accountInfo->GameAccounts[fields[0].Get<uint32>()]
                    .LastPlayedCharacters[realmId.GetSubRegionAddress()];

                lastPlayedCharacter.RealmId = realmId;
                lastPlayedCharacter.CharacterName = fields[4].Get<std::string>();
                lastPlayedCharacter.CharacterGUID = fields[5].Get<uint64>();
                lastPlayedCharacter.LastPlayedTime = fields[6].Get<uint32>();
        ...
        {
            LOG_DEBUG("session", "[Session::HandleVerifyWebCredentials] Account '{}' is locked to IP - '{}' is logging in from '{}'",
        ...
        else
        {
            ...
            LOG_DEBUG("session", "[Session::HandleVerifyWebCredentials] Account '{}' is not locked to ip", _accountInfo->Login);
            if (_accountInfo->LockCountry.empty() || _accountInfo->LockCountry == "00")
                LOG_DEBUG("session", "[Session::HandleVerifyWebCredentials] Account '{}' is not locked to country", _accountInfo->Login);
            else if (!_accountInfo->LockCountry.empty() && !_ipCountry.empty())
            {
                LOG_DEBUG("session", "[Session::HandleVerifyWebCredentials] Account '{}' is locked to country: '{}' Player country is '{}'",
        ...
        if (_accountInfo->IsBanned)
        {
            ...
            {
                LOG_DEBUG("session", "{} [Session::HandleVerifyWebCredentials] Banned account {} tried to login!", GetClientInfo(), _accountInfo->Login);
                asyncContinuation(&asyncContinuationService, ERROR_GAME_ACCOUNT_BANNED, &response);
                return;
            }
            else
            {
                LOG_DEBUG("session", "{} [Session::HandleVerifyWebCredentials] Temporarily banned account {} tried to login!", GetClientInfo(), _accountInfo->Login);
        ...
        std::array<uint8, 64> k = Acore::Crypto::GetRandomBytes<64>();
...
uint32 Battlenet::Session::HandleProcessClientRequest(game_utilities::v1::ClientRequest const* request, game_utilities::v1::ClientResponse* response)
{
    ...
    if (!command)
    {
        LOG_ERROR("session.rpc", "{} sent ClientRequest with no command.", GetClientInfo());
    ...
    {
        LOG_ERROR("session.rpc", "{} sent ClientRequest with unknown command {}.", GetClientInfo(), removeSuffix(command->name()));
...
uint32 Battlenet::Session::GetRealmListTicket(std::unordered_map<std::string, Variant const*> const& params, game_utilities::v1::ClientResponse* response)
{
    ...
    stmt->SetData(1, GetLocaleByName(_locale));
    stmt->SetData(2, _os);
    stmt->SetData(3, _accountInfo->Id);
...
void Battlenet::Session::HandshakeHandler(boost::system::error_code const& error)
{
    if (error)
    {
        LOG_ERROR("session", "{} SSL Handshake failed {}", GetClientInfo(), error.message());
        CloseSocket();
        return;
    }
