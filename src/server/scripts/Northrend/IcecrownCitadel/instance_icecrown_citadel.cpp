        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            switch (go->GetEntry())
            {
                ...
                case GO_SCIENTIST_ENTRANCE:
                    ...
                    // con Putricidio muerto la puerta queda abierta aunque la trampa no terminara (la ignora un GM)
                    HandleGameObject(PutricideEnteranceDoorGUID, (PutricideEventProgress & PUTRICIDE_EVENT_FLAG_TRAP_FINISHED) || GetBossState(DATA_PROFESSOR_PUTRICIDE) == DONE, go);
                    break;
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            switch (type)
            {
                ...
                case DATA_ROTFACE:
                    ...
                    break;
                case DATA_PROFESSOR_PUTRICIDE:
                    HandleGameObject(PutricideEnteranceDoorGUID, state != IN_PROGRESS && (state == DONE || (PutricideEventProgress & PUTRICIDE_EVENT_FLAG_TRAP_FINISHED)));
