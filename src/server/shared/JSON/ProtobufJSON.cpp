...
bool Deserializer::Key(const Ch* str, rapidjson::SizeType /*length*/, bool /*copy*/)
{
    ...
    if (!field)
    {
        _errors.push_back(Acore::StringFormat("Message {} has no field {}.", _objectState.top()->GetTypeName(), str));
        return false;
    }
...
bool Deserializer::Uint(uint32 i)
{
    ...
    switch (field->cpp_type())
    {
        ...
        default:
            _errors.push_back(Acore::StringFormat("Expected field type to be uint32 or string but got {} instead.", _state.top()->cpp_type_name()));
            return false;
    }
...
bool Deserializer::Double(double d)
{
    ...
    switch (field->cpp_type())
    {
        ...
        case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
            ...
            break;
        default:
            _errors.push_back(Acore::StringFormat("Expected field type to be float or double but got {} instead.", _state.top()->cpp_type_name()));
            return false;
    }
...
bool Deserializer::String(const Ch* str, rapidjson::SizeType /*length*/, bool /*copy*/)
{
    ...
    switch (field->cpp_type())
    {
        case google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
        {
            ...
            if (!enumValue)
            {
                _errors.push_back(Acore::StringFormat("Field {} enum {} does not have a value named {}.", field->full_name(), field->enum_type()->full_name(), str));
                return false;
            }
        ...
        case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
            ...
            break;
        default:
            _errors.push_back(Acore::StringFormat("Expected field type to be string or enum but got {} instead.", _state.top()->cpp_type_name()));
            return false;
    }
...
bool Deserializer::StartObject()
{
    ...
    if (!_state.empty())
    {
        ...
        {
            _errors.push_back(Acore::StringFormat("Expected field {} to be a message but got {} instead.", _state.top()->name(), _state.top()->cpp_type_name()));
            return false;
        }
...
bool Deserializer::StartArray()
{
    ...
    {
        _errors.push_back(Acore::StringFormat("Expected field {} type to be exactly an array OR bytes but it was both or none.", _state.top()->full_name()));
        return false;
    }
...
bool Deserializer::CheckType(google::protobuf::FieldDescriptor::CppType expectedType)
{
    ...
    {
        _errors.push_back(Acore::StringFormat("Expected field {} type to be {} but got {} instead.",
...
bool JSON::Deserialize(std::string const& json, google::protobuf::Message* message)
{
    ...
    if (!deserializer.ReadMessage(json, message))
    {
        for (std::size_t i = 0; i < deserializer.GetErrors().size(); ++i)
            LOG_ERROR("json", "{}", deserializer.GetErrors()[i]);
        return false;
    }
