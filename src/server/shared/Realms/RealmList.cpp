...
void RealmList::UpdateRealms()
{
    ...
    LoginDatabasePreparedStatement *stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_REALMLIST_BNET);
    ...
    if (result)
    {
        do
        {
            try
            {
                ...
                if (!externalAddress)
                {
                    LOG_ERROR("realmlist", "Could not resolve address {} for realm \"{}\" id {}", externalAddressString, name, realmId);
                    continue;
                }
                ...
                if (!localAddress)
                {
                    LOG_ERROR("realmlist", "Could not resolve localAddress {} for realm \"{}\" id {}", localAddressString, name, realmId);
                    continue;
                }

                uint16 port = fields[4].Get<uint16>();
                uint8 icon = fields[5].Get<uint8>();
                if (icon == REALM_TYPE_FFA_PVP)
                    icon = REALM_TYPE_PVP;
                if (icon >= MAX_CLIENT_REALM_TYPE)
                    icon = REALM_TYPE_NORMAL;
                RealmFlags flag = RealmFlags(fields[6].Get<uint8>());
                uint8 timezone = fields[7].Get<uint8>();
                uint8 allowedSecurityLevel = fields[8].Get<uint8>();
                float pop = fields[9].Get<float>();
                uint32 build = fields[10].Get<uint32>();
                uint8 region = fields[11].Get<uint8>();
                uint8 battlegroup = fields[12].Get<uint8>();
                ...
                if (!existingRealms.count(id))
                    LOG_INFO("realmlist", "Added realm \"{}\" at {}:{}.", name, externalAddressString, port);
                else
                    LOG_DEBUG("realmlist", "Updating realm \"{}\" at {}:{}.", name, externalAddressString, port);

                existingRealms.erase(id);
            }
            catch (std::exception& ex)
            {
                LOG_ERROR("realmlist", "Realmlist::UpdateRealms has thrown an exception: {}", ex.what());
                ABORT();
            }
        }
        while (result->NextRow());
    ...
    for (auto itr = existingRealms.begin(); itr != existingRealms.end(); ++itr)
        LOG_INFO("realmlist", "Removed realm \"{}\".", itr->second);
...
Realm const* RealmList::GetRealm(Battlenet::RealmHandle const& id) const
{
    ...
    return Acore::Containers::MapGetValuePtr(_realms, id);
}

bool RealmList::GetRealmNames(Battlenet::RealmHandle const& id, std::string* name, std::string* normalizedName) const
{
    std::shared_lock<std::shared_mutex> lock(_realmsMutex);
    Realm const* realm = Acore::Containers::MapGetValuePtr(_realms, id);
    if (!realm)
        return false;
    ...
    LocaleConstant locale, std::string const& os, Minutes timezoneOffset, std::string const& accountName, bgs::protocol::game_utilities::v1::ClientResponse* response) const
{
    ...
    if (Realm const* realm = GetRealm(Battlenet::RealmHandle(realmAddress)))
    {
        ...
        *reinterpret_cast<uint32*>(compressed.data()) = uint32(json.length() + 1);
        ...
        stmt->SetData(3, os);
        stmt->SetData(4, timezoneOffset.count());
        stmt->SetData(5, accountName);
