...
{
    if (auto addressIndex = Acore::Net::SelectAddressForClient(clientAddr, Addresses))
...
{
    return Acore::StringFormat("{}-{}-{}", Region, Site, Realm);
}

std::string Battlenet::RealmHandle::GetSubRegionAddress() const
{
    return Acore::StringFormat("{}-{}-0", Region, Site);
}
