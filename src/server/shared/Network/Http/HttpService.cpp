...
{
    LOG_DEBUG(_logger, "{} Starting request {} {}", session->GetClientInfo(),
    ...
    HttpMethodHandlerMap& handlerMap = [&]() -> HttpMethodHandlerMap&
    {
        switch (method)
        {
            ...
            default:
            {
                ...
                WPError(false, "Tried to register a handler for unsupported HTTP method"); (void)methodString;
            }
        }
    }();

    handlerMap[std::string(path)] = { .Func = std::move(handler), .Flags = flags };
    LOG_INFO(_logger, "Registered new handler for {} {}", ToStdStringView(boost::beast::http::to_string(method)), path);
}
...
void SessionService::InitAndStoreSessionState(std::shared_ptr<SessionState> state, boost::asio::ip::address const& address)
{
    ...
    // Generate session id
    {
        ...
        while (state->Id.is_nil() || _sessions.contains(state->Id))
            std::copy_n(Acore::Crypto::GetRandomBytes<16>().begin(), 16, state->Id.begin());

        LOG_DEBUG(_logger, "Client at {} created new session {}", address.to_string(), boost::uuids::to_string(state->Id));
...
void SessionService::Start(Asio::IoContext& ioContext)
{
    ...
    _inactiveSessionsKillTimer->expires_after(std::chrono::minutes(1));
...
std::shared_ptr<SessionState> SessionService::FindAndRefreshSessionState(std::string_view id, boost::asio::ip::address const& address)
{
...

    {
        ...
        {
            LOG_DEBUG(_logger, "Client at {} attempted to use a session {} that was expired", address.to_string(), id);
    ...
    {
        LOG_ERROR(_logger, "Client at {} attempted to use a session {} that was last accessed from {}, denied access",
...
void SessionService::MarkSessionInactive(boost::uuids::uuid const& id)
{
    ...
    if (wasActive)
    {
        ...
        if (itr != _sessions.end())
        {
            ...
            LOG_TRACE(_logger, "Session {} marked as inactive", boost::uuids::to_string(id));
        }
    }
}
...
void SessionService::KillInactiveSessions()
{
...

    {
        ...
        LOG_DEBUG(_logger, "Killed {} inactive sessions", inactiveSessionsCount - inactiveSessions.size());
    }
