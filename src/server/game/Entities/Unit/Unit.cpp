...
uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss, bool /*allowGM*/, Spell const* damageSpell /*= nullptr*/)
{
    ...
    // mod-spellregulator: aplicar porcentaje SOLO a DOT.
    // SPELL_DIRECT_DAMAGE se amplifica en SendSpellNonMeleeDamageLog (modifica
    // damageInfo.damage por referencia y se propaga a este DealDamage).
    // Si amplificasemos aqui tambien, seria doble.
...
{
    // mod-spellregulator: aplicar porcentaje al damage logueado al cliente
    if (log->spellInfo)
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    // mod-spellregulator: aplicar porcentaje al damage de auras periodicas (DoTs/HoTs)
...
int32 Unit::HealBySpell(HealInfo& healInfo, bool critical)
{
    ...
    // mod-spellregulator: aplica porcentaje al heal final, DESPUES de IP y bonuses
    if (healInfo.GetSpellInfo())
