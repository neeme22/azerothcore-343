...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recv_data)
{
    ...
    // Skip areatrigger scripts for GMs unless debug areatriggers is enabled or GM.AreaTriggerScripts is set
    // (e.g. the ICC portal behind Deathbringer's door, which only teleports through its script)
    static bool const gmAreaTriggerScripts = sConfigMgr->GetOption<bool>("GM.AreaTriggerScripts", true);
    if ((!player->IsGameMaster() || player->isDebugAreaTriggers || gmAreaTriggerScripts) && sScriptMgr->OnAreaTrigger(player, atEntry))
        return;
