...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    // mod-spellregulator: aplicar porcentaje a amounts de auras NO-periodicas
    // (las periodicas se amplifican en SendPeriodicAuraLog + DealDamage(DOT)/heal)
    // Esto cubre: escudos/absorb, stat reductions/buffs, slows, mod damage/heal done,
    // resistance changes, attack power, armor%, etc.
    {
        AuraType at = GetAuraType();
        bool periodicHandledElsewhere =
            (at == SPELL_AURA_PERIODIC_DAMAGE ||
             at == SPELL_AURA_PERIODIC_DAMAGE_PERCENT ||
             at == SPELL_AURA_PERIODIC_LEECH ||
             at == SPELL_AURA_PERIODIC_HEAL ||
             at == SPELL_AURA_OBS_MOD_HEALTH ||
             at == SPELL_AURA_PERIODIC_HEALTH_FUNNEL ||
             at == SPELL_AURA_PERIODIC_MANA_LEECH ||
             at == SPELL_AURA_OBS_MOD_POWER ||
             at == SPELL_AURA_PERIODIC_ENERGIZE ||
             at == SPELL_AURA_POWER_BURN ||
             at == SPELL_AURA_PERIODIC_DUMMY ||
             at == SPELL_AURA_PERIODIC_TRIGGER_SPELL ||
             at == SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE);
        if (!periodicHandledElsewhere)
        {
            // sign-preserving (handles negative amounts: slows, stat reductions)
            bool neg = amount < 0;
            uint32 tmp = uint32(neg ? -amount : amount);
            sSpellRegulator->Regulate(tmp, GetId(), caster, "CalcAmt");
            amount = neg ? -int32(tmp) : int32(tmp);
        }
    }
