...
#define BnetGameAccountInfo "a.id, a.username, ab.unbandate, ab.unbandate = ab.bandate, aa.SecurityLevel"
    // --- bnet 3.4.3 (portado de TrinityCore)
    PrepareStatement(LOGIN_SEL_REALMLIST_BNET, "SELECT id, name, address, localAddress, port, icon, flag, timezone, allowedSecurityLevel, population, gamebuild, Region, Battlegroup FROM realmlist WHERE flag <> 3 ORDER BY name", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_INS_BNET_ACCOUNT_FOR_ACCOUNT, "INSERT INTO battlenet_accounts (email, srp_version, salt, verifier) VALUES (?, 0, ?, ?)", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_UPD_ACCOUNT_LINK_BNET, "UPDATE account SET battlenet_account = ?, battlenet_index = 1 WHERE id = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_BNET_AUTHENTICATION, "SELECT a.id, 0, a.salt, a.verifier, a.failed_logins, ba.LoginTicket, ba.LoginTicketExpiry, (ab.unbandate > UNIX_TIMESTAMP() OR ab.unbandate = ab.bandate), ba.id FROM account a LEFT JOIN battlenet_accounts ba ON a.battlenet_account = ba.id LEFT JOIN account_banned ab ON a.id = ab.id AND ab.active = 1 WHERE a.username = ?", CONNECTION_ASYNC);
