...
// worldgate: paradas de ascensores (GAMEOBJECT_TYPE_TRANSPORT) que la pasarela crea sin pasar por GameObject::Create, así que
// sin GameObjectType::Transport. Sin ellas el bloque de creación no lleva PauseTimes y el cliente cicla el ascensor siempre
TC_GAME_API std::unordered_map<GameObject const*, std::vector<uint32>>& WorldgatePausas()
{
    static std::unordered_map<GameObject const*, std::vector<uint32>> pausas;
    return pausas;
}
...
std::vector<uint32> const* GameObject::GetPauseTimes() const
{
    ...
    auto& pausas = WorldgatePausas();
    auto it = pausas.find(this);
    if (it != pausas.end())
        return &it->second;
