    ...
    *data << uint32(PauseTimes ? PauseTimes->size() : 0);
    ...
    if (flags.Vehicle)
    {
        ...
        *data << uint32(unit->GetVehicleKit()->GetVehicleInfo()->ID);   // RecID
        // worldgate: sobre un transporte la orientación de partida es la relativa a él, como en AC/TC 3.3.5. Con la de mundo
        // el cañón de la cañonera de ICC salía con el giro de la nave sumado (180°) y el límite de ±40° lo volteaba
        *data << float(!unit->m_movementInfo.transport.guid.IsEmpty() ? unit->GetTransOffsetO() : unit->GetOrientation()); // InitialRawFacing
    }
    ...
    if (flags.ActivePlayer)
    {
        ...
        bool HasRuneState = player->GetClass() == CLASS_DEATH_KNIGHT;   // worldgate: runas del 3.3.5 (como Wrathion); el estado real llega con SMSG_RESYNC_RUNES
        ...
        if (HasRuneState)
        {
            *data << uint8(0x3F);                        // worldgate: 6 runas, todas listas
            *data << uint8(0x3F);
            *data << uint32(6);
