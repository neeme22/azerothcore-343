...
WorldPacket const* WorldPackets::Pet::PetSpells::Write()
{
    ...
    _worldPacket << uint16(CommandState | (Flag << 24));
    ...
    for (PetSpellCooldown const& cooldown : Cooldowns)
    {
        ...
        _worldPacket << uint32(cooldown.Duration);
        _worldPacket << uint32(cooldown.CategoryDuration);
    ...
    {
        _worldPacket << uint32(history.CategoryID);
        _worldPacket << uint32(history.RecoveryTime);
