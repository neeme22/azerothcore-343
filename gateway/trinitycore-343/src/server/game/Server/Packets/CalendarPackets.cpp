...
ByteBuffer& operator<<(ByteBuffer& data, WorldPackets::Calendar::CalendarSendCalendarRaidResetInfo const& resetsInfo)
{
    data << int32(resetsInfo.MapID);
    data << uint32(resetsInfo.Unk340_1);
    data << int32(resetsInfo.IntervalSeconds);
    data << int32(resetsInfo.OffsetSeconds);
    data << int32(resetsInfo.Difficulty);
    return data;
}
...
WorldPacket const* WorldPackets::Calendar::CalendarSendCalendar::Write()
{
    ...
    // alistar: TODO move me to instance reset
    {
        RaidResets.push_back({ 309, 0, 259200, 68400, 148 });
        RaidResets.push_back({ 509, 0, 259200, 68400, 148 });

        RaidResets.push_back({ 531, 0, 604800, 0, DIFFICULTY_40 });
        RaidResets.push_back({ 469, 0, 604800, 0, DIFFICULTY_40 });
        RaidResets.push_back({ 409, 0, 604800, 0, DIFFICULTY_40 });

        RaidResets.push_back({ 532, 0, 604800, 0, DIFFICULTY_10_N });

        RaidResets.push_back({ 534, 0, 604800, 0, DIFFICULTY_25_N });
        RaidResets.push_back({ 544, 0, 604800, 0, DIFFICULTY_25_N });
        RaidResets.push_back({ 548, 0, 604800, 0, DIFFICULTY_25_N });
        RaidResets.push_back({ 550, 0, 604800, 0, DIFFICULTY_25_N });
        RaidResets.push_back({ 564, 0, 604800, 0, DIFFICULTY_25_N });
        RaidResets.push_back({ 565, 0, 604800, 0, DIFFICULTY_25_N });

        RaidResets.push_back({ 568, 0, 259200, 0, DIFFICULTY_10_N });

        RaidResets.push_back({ 580, 0, 604800, 0, DIFFICULTY_25_N });
        RaidResets.push_back({ 616, 0, 604800, 0, DIFFICULTY_10_N });
        RaidResets.push_back({ 533, 0, 604800, 0, DIFFICULTY_10_N });
        RaidResets.push_back({ 615, 0, 604800, 0, DIFFICULTY_10_N });
        RaidResets.push_back({ 624, 0, 604800, 0, DIFFICULTY_10_N });
    }

    _worldPacket << uint32(RaidResets.size());

    for (CalendarSendCalendarRaidResetInfo const& resets : RaidResets)
        _worldPacket << resets;
