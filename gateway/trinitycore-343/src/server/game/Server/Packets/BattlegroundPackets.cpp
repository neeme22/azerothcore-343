...
ByteBuffer& operator<<(ByteBuffer& data, WorldPackets::Battleground::PVPMatchStatistics const& pvpLogData)
{
    ...
    data.WriteBit(false); // HasNames (ArenaTeams no longer in 3.4.3) 
...
{
    _worldPacket >> PackedBattlegroundQueueTypeID;
    _worldPacket >> Roles;
    _worldPacket >> BlacklistMap[0];
    _worldPacket >> BlacklistMap[1];
    _worldPacket >> BattlemasterGuid;
    _worldPacket >> UnkID;
    _worldPacket >> BattlefieldIndexSpecific;
    JoinAsGroup = _worldPacket.ReadBit();
    QueueIDs.push_back(PackedBattlegroundQueueTypeID);   // para el código de TC que lee QueueIDs
}
...
{
    _worldPacket << BattlemasterGuid;
    _worldPacket << int32(Verification);
...
void WorldPackets::Battleground::BattlemasterJoinSkirmishArena::Read()
{
    _worldPacket >> BattlemasterGuid;
    _worldPacket >> Roles;
    _worldPacket >> Bracket;
