...
Player* RandomPlayerbotFactory::CreateRandomBot(WorldSession* session, uint8 cls, std::unordered_map<NameRaceAndGender, std::vector<std::string>>& nameCache)
{
    ...
    // Guard against missing character appearance data (e.g. custom/incomplete CharSections.dbc).
    // Without this, an empty faces/hairs vector makes urand(0, size()-1) underflow and the
    // index access below crashes with ACCESS_VIOLATION. Skip the bot instead of crashing.
    if (faces.empty() || hairs.empty())
    {
        LOG_ERROR("playerbots", "Missing CharSections appearance data (faces/hairs) for race: {}, gender: {} - skipping bot (check your DBC)", race, gender);
        return nullptr;
    }
