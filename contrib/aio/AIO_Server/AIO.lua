...
if AIO_SERVER then
    ...
    -- 3.4.3+: CHAT_MSG_ADDON da el emisor como "Nombre-Reino"; 3.3.5 da "Nombre".
    local function AIO_IsSelf(sender)
        local me = UnitName("player")
        if sender == me then return true end
        return Ambiguate ~= nil and sender ~= nil and Ambiguate(sender, "none") == me
    end
    local function ONADDONMSG(self, event, prefix, msg, Type, sender)
        if prefix == AIO_ServerPrefix then
            if event == "CHAT_MSG_ADDON" and AIO_IsSelf(sender) then
    ...
    function frame:OnEvent(event, addon)
        if event == "ADDON_LOADED" and addon == "AIO_Client" then
            ...
            -- 3.4.3 (Wrath Classic) da 30403 pero exige registrar prefijos como cata+
            if tocversion and (tocversion >= 40100 or C_ChatInfo) and RegisterAddonMessagePrefix then
                RegisterAddonMessagePrefix("C"..AIO_Prefix)
                RegisterAddonMessagePrefix("S"..AIO_Prefix)
            end
